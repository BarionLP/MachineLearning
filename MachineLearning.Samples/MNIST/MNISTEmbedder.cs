...
public sealed class MNISTEmbedder(ImmutableArray<int> _nodeMapping) : IEmbedder<float[], int>
{
    ...
    public Vector Embed(float[] input) => Vector.Of(input);
    ...
    public Vector Embed(float[] input, ILayerSnapshot snapshot)
    {
        return Embed(input);
    }

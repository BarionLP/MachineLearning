...
public sealed class TrainingEvaluationResult
{
    ...
    public override string ToString() => $"Correct: {Result.CorrectPercentage:P0} ({Result.AverageCost:F4})\t({Result.AverageElapsedTime:ss\\.ff}s/batch\t{Context} ({Result.AverageCount}))";
}

...
where TSnapshot : ILayerSnapshot
{
    ...
    public GenericModelTrainer(TModel model, TrainingConfig config, ITrainingSet trainingSet)
    {
        Config = config;
    ...
    public void Train(CancellationToken? token = null)
    {
        ...
        foreach (var (epochIndex, epoch) in TrainerHelper.GetEpochs(TrainingSet, Config.EpochCount).Index())
        {
            ...
            {
                cachedEvaluation += TrainAndEvaluate(batch.Cast<TrainingData<TArch, TArch>>());
    ...
    public DataSetEvaluationResult TrainAndEvaluate(IEnumerable<TrainingData<TArch, TArch>> trainingBatch)
    {
        ...
        void UpdateCounters(TrainingData<TArch, TArch> data, TArch weights)
        {
            dataCounter++;
    ...
    private TArch Update(TrainingData<TArch, TArch> data)
    {

...
public sealed class EmbeddedMamba2LayerAdam : ILayerOptimizer<EmbeddedMamba2Layer, EmbeddedMamba2Layer.Snapshot>
{
    public EmbeddedMamba2Layer Layer { get; }
    public ICostFunction CostFunction => Optimizer.CostFunction;
    public AdamOptimizer Optimizer { get; }
    ...
    public EmbeddedMamba2LayerAdam(AdamOptimizer optimizer, EmbeddedMamba2Layer layer)
    {
        Optimizer = optimizer;
        Layer = layer;

        GradientAlpha = Vector.Create(Layer.SequenceLength);
        GradientB = Matrix.OfSize(Layer.B);
        GradientC = Matrix.OfSize(Layer.C);

        FirstMomentAlpha = Vector.Create(Layer.SequenceLength);
        FirstMomentB = Matrix.OfSize(Layer.B);
        FirstMomentC = Matrix.OfSize(Layer.C);

        SecondMomentAlpha = Vector.Create(Layer.SequenceLength);
        SecondMomentB = Matrix.OfSize(Layer.B);
        SecondMomentC = Matrix.OfSize(Layer.C);
    }

    private readonly Lock _lock = new();
    public void Update(Vector nodeValues, EmbeddedMamba2Layer.Snapshot snapshot)
    {
        Layer.BackwardPass(snapshot, Matrix.Of(Layer.SequenceLength, Layer.EmbeddingDimensions, nodeValues));
